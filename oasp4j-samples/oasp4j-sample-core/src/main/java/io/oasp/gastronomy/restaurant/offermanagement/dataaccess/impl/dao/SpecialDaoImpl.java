...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  public PaginatedListTo<SpecialEntity> findSpecials(SpecialSearchCriteriaTo criteria) {
    ...
    if (activePeriodTo != null) {

      // mind that this is not the correct business logic
      query.where(Alias.$(special.getActivePeriod().getStartingHour()).eq(activePeriodTo.getStartingHour()));
      query.where(Alias.$(special.getActivePeriod().getEndingHour()).eq(activePeriodTo.getEndingHour()));

      if (activePeriodTo.getStartingDay() != null) {
        query.where(Alias.$(special.getActivePeriod().getStartingDay()).eq(activePeriodTo.getStartingDay()));
      }

      if (activePeriodTo.getEndingDay() != null) {
        query.where(Alias.$(special.getActivePeriod().getEndingDay()).eq(activePeriodTo.getEndingDay()));
      }
    }
